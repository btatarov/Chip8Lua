...
function class:__call(...)
    ...
    -- Extending objects
    clazz.extend = function(func)
        func(clazz, table.copy(clazz))
    end
...
--------------------------------------------------------------------------------
-- Converts string to a table containing each char.
-- @return table
--------------------------------------------------------------------------------
function string:toTable()
    ...
    for i = 1, self:len() do
        table.insert(chunks, self:sub(i, i))
    end
...
function Resources.getTextureAtlas(luaFilePath, texture)
    ...
    for i, frame in ipairs(frames) do
        ...
        dataFrame.textureRotated = frame.textureRotated
...
function DisplayObject:setPosCenter(left, top)
    -- preserve values for resolution updates
    self.rel_left = left
    self.rel_top = top

    local viewWidth, viewHeight
    if self.parent then
        viewWidth, viewHeight = self.parent:getDims()
    else
        viewWidth, viewHeight = M.viewWidth, M.viewHeight
    end

    local left = left + viewWidth / 2 - self:getWidth() / 2
    local top = top + viewHeight / 2 - self:getHeight() / 2
...
function Group:scissorRect(rect)
    self:setScissorRect(rect)

    for i, v in ipairs(self.children) do
        if v.scissorRect then
            v:scissorRect(rect)
        else
            v:setScissorRect(rect)
        end
    end
end
...
function Label:init(text, width, height, font, textSize)
    ...
    font = Resources.getFont(font, nil, textSize * M.viewScale)

    self.style = self:affirmStyle()
    self.style:setFont(font)
    self.style:setSize(textSize * M.viewScale)
    self.style:setScale(1 / M.viewScale)
...
-- @param height Height
--------------------------------------------------------------------------------
function Rect:init(width, height, color)
    DisplayObject.init(self)
...
----------------------------------------------------------------------------------------------------
Texture = class()
Texture.__factory = MOAIImageTexture
M.Texture = Texture

--- Default Texture filter and Image transform
Texture.DEFAULT_FILTER = MOAITexture.GL_LINEAR
Texture.DEFAULT_TRANSFORM = MOAIImage.PREMULTIPLY_ALPHA
...
function Texture:init(path)
    self:load(path, Texture.DEFAULT_TRANSFORM)
    self.path = path
...
function Font:init(path, charcodes, points, dpi)
    ...
    if Font.DEFAULT_FILTER then
        self:setFilter(Font.DEFAULT_FILTER)
    end
end
...
function TouchHandler:getTouchableProp(e)
    ...
    for i = #props, 1, -1 do
        local prop = props[i]
        if prop:getAttr(MOAIProp.ATTR_VISIBLE) > 0 and not prop.isNotTouchable == true then
            return prop
        end
    end
end
